...
var TelemetryReporter = /** @class */ (function () {
    ...
    TelemetryReporter.prototype.sendTelemetryMetric = function (metricName, value, properties, count, min, max, stdDev) {
        if (this.userOptIn && metricName && this.appInsightsClient) {
            ...
            if (this.logStream) {
                this.logStream.write("telemetry/" + metricName + " " + value + " " + JSON.stringify({ properties: properties, count: count, min: min, max: max, stdDev: stdDev }) + "\n");
            }
        }
    };
